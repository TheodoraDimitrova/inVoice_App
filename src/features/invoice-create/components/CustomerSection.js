...
import { Autocomplete, MenuItem, Switch, TextField } from "@mui/material";
...
export const CustomerSection = ({
  ...
  const showSavedCustomers =
    Array.isArray(savedCustomerOptions) && savedCustomerOptions.length > 0;
        ...
        <div className="col-span-12 md:col-span-6">
          {showSavedCustomers ? (
            <Autocomplete
              ...
              onChange={onCustomerNameAutocompleteChange}
              onInputChange={onCustomerNameAutocompleteInputChange}
              getOptionLabel={(option) =>
                typeof option === "string"
                  ? option
                  : String(option?.customerName || "").trim() || "—"
              }
              isOptionEqualToValue={(a, b) => (a?.id ?? "") === (b?.id ?? "")}
              noOptionsText="Няма съвпадения между записани"
              renderInput={(params) => (
                <TextField
                  {...params}
                  {...fieldProps}
                  label="Име на клиент"
                  name="customerName"
                  placeholder="Изберете записан клиент или въведете име"
                  required
                    ...
                    <FormFieldHelperText
                      ...
                      hint={
                        customerType === "business"
                          ? "Записани фирми за този тип или ново име."
                          : "Записани физически лица за този тип или ново име."
                      }
                    />
                  }
